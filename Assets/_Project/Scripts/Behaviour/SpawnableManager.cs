using UnityEngine;
using BoundingArea = ObjectSpawner.BoundingArea;
...
public class SpawnableManager : BehaviourClass
{
    ...
    void Update()
    {
        ...
        if (_BoundingArea == BoundingArea.Controller)
    ...
    {
        if (_BoundingArea == BoundingArea.Unrestricted)
            return false;
    ...
    private void DefineBoundingRules()
    {
        ...
        {
            case BoundingArea.Spawn:
                _BoundingCentre = _SpawnPosition;
                break;
            case BoundingArea.ControllerBounds:
